...
pub fn get_diff_sections(
    target_branch: &str,
    verbose: bool,
) -> Result<Vec<Section>, crate::error::Error> {
    diff(target_branch, verbose).map(|diff| sections(&diff))
}

fn diff(target: &str, verbose: bool) -> Result<String, crate::error::Error> {
...
fn sections(git_diff: &str) -> Vec<Section> {
